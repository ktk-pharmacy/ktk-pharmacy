    ...
    <div class="container">
      <div class="row">
        ...
        <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
         @foreach($maincategories as $mcat)
          <button class="nav-link bg-red mb-2 button {{$mcat->id ? 'active': '' }}"
                  ...
                  aria-controls="v-pills-{{$mcat->id}}" @if($mcat->id=='#v-pills-$mcat->id') aria-selected="true" @else aria-selected="false" @endif>{{$mcat->name}}</button>
         @endforeach
        ...
        <div class="tab-content" id="v-pills-tabContent">
         @foreach($maincategories as $mcat)
           <div class="tab-pane fade  {{$mcat->count()!=0 ? 'show active': '' }}" id="v-pills-{{$mcat->id}}" role="tabpanel" aria-labelledby="v-pills-{{$mcat->id}}-tab"> <div class="row">
                      ...
                      <div class="col-lg-4 mb-4">
                        <div class="meeting-item">
                          <div class="thumb">
                            ...
                            <a href="{{url('products',$subcat->id)}}"><img  width="416" height="284" src="{{$subcat->image_url}}" alt="Allergy & Immune Care"></a>
                          </div>
                          <div class="down-content">
                            <a href="{{ url('products',$subcat->id)}}"><h4 class="justify-center">{{$subcat->name}}</h4></a>
                          </div>
                        </div>
                      </div>
                    @endforeach
