        ...
        <div class="card">
            <div class="card-body tb-card">
                ...
                <div class="col-lg-6 mb-7 right py-4 flex ml-auto">
                    @can('create')
                        <button data-url="{{ route('hero_slider_create') }}"
                        ...
                        <tbody>
                            @foreach ($sliders as $key => $slider)
                                <tr>
                                    <td>{{ ++$key }}</td>
                                    <td class="py-1">
                                        <img style="width:80px;height:40px;" src="{{ $slider->image }}"
                                            alt="image" />
                                    </td>
                                    <td>
                                        @can('edit')
                                          <a href="javascript:void(0)"
                                          data-url="{{ route('hero_slider_edit', $slider->id) }}" class="mx-2 form-btn"><i
                                            class="fa-regular fa-pen-to-square"></i></a>
                                        @endcan
                                        @can('delete')
                                        <a href="javascript:void(0)"
                                        data-url="{{ route('hero_slider_destroy', $slider->id) }}"
